...
\end{equation} \todo{zavest matematicky symbol pokud chci pouzitlimits}
...
Using them on visualisable benchmark datasets with known problems helped to determine their properties.
...
\todo{make references to each table and picture or put hard placement for them?}
...
A method to represent clusters in non-metric space by a small subset of the whole cluster was implemented and tested in this thesis.
...
The experiments that were performed have shown that the $\delta$-Medoids Modified fulfills the requirements the best.
It selects a small number of representatives from each cluster and is more precise than currently used Random Selection algorithm.
The average fit by representatives of the data was also tested.
A cluster prototype created by $\delta$-Medoids algorithm fits clusters significantly better than randomly selected representatives.

This thesis explored the nature of the clusters collected on the Internet.
