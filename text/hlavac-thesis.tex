...
\begin{itemize}
    ...
    \item Overlap
    \item Noisy Circles
    \item Noisy Circles 3D \todo{add spiral dataset}
\end{itemize}
...
The Pen-Based Recognition of Handwritten Digits Data Set \cite{dua1998pendigit} consists of \todo{explain what it is}
...
\todo{Struktura popisu jednotlivych experimantu jako sekce.
1) motivace toho experiment - proc delame ten experiment, co chceme zjistit
2) experimental setup - jaky datset + jake algorimty a jejich nastaveni
3) komentovane vysledky
4) shrnuti co jsme se dozvedeli}
...
\todo{where to put cross-validation}
...
The numbers of representatives selected via random selection algorithm are decided by the size of results of full $\delta$-Medoids. \todo{Random selection repetition??}
...
As explained in Section \todo{Medoid!} all of these representatives are medoids - they are part of the clusters.
...
The $K$ selected for testing is 1. \todo{test other K, and then explain why 1}
