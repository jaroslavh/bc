...
\abstractEN{
Making computer networks secure is essential today as the Internet is the dominant medium for communication. One approach for securing networks is monitoring their behavior patterns. Precise and fast network behavior anomaly detection highly depends on the models created for the monitored network. To create a model that is not clogged by noise from the whole network, hosts can be separated into smaller groups by community-based clustering. On big networks of tens of thousands, these clusters can contain several thousands of hosts.

This thesis presents an algorithm for finding cluster prototypes for clusters in Cognitive Targeted Anomaly Detection Framework. In the framework, representatives are currently selected randomly. Because of the dynamic nature of networks, only a pair-wise similarity exists between each host pair. Therefore, representative selection methods for topological spaces have to be used. A modification to $\delta$-Medoids algorithm is proposed with the ability to select a low number of representatives while maintaining the best possible coverage of the cluster. This algorithm improves the coverage of clusters in real network dataset by more than 4\% as compared to selecting the representatives randomly.}
...
\end{tabular}}
\caption{Number of selected representatives for Circles 3D dataset}
\end{table}
...
\begin{figure}[t]
  ...
  \label{img:exp4}
\end{figure}
\todo{get current images and data here}
