...
\abstractEN{
Making computer networks secure is essential today as the Internet is the dominant medium for communication.
...
In the framework, a number of samples is currently selected randomly as a representation of a cluster.
Because of the dynamic nature of networks, only a pair-wise similarity exists between each host pair.
Therefore, representative selection methods for topological spaces have to be used.
A modification to $\delta$-Medoids algorithm is proposed with the ability to select a low number of representatives while maintaining the best possible coverage of the cluster.
This algorithm improves the coverage of clusters in real network dataset by more than 4\% as compared to selecting the representatives randomly.
}
\abstractCS{
...
Při použití na reálných síťových datech, tento algoritmus vylepšuje pokrytí shluku jeho reprezentanty minimálně o 4\%.
...
\keywordsEN{Representative selection, Network anomaly detection, Community-based clustering, Intrusion detection system, $\delta$-Medoids algorithm, Topological space, Pair-wise similarity}
