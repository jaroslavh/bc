...
However, if a calculated centroid is not meaningful (see Figure~\ref{img:centroids} b)\todo{references not working? why?}), a medoid can be selected as an alternative to it.
...
\begin{figure}[H]
  ...
  \caption{Confusion matrices for Pendigits dataset}
  \label{img:exp4}
\end{figure} \todo{here add better cluster names for sure}

\subsection{Conclusion}
The results clearly show that the $\delta$-Medoids algorithm is significanly better than selecting random representatives from the cluster.
The downside is the speed of the algorithm. \todo{expand the conclusion}
