...
\section{Challenges of Real-Time Anomaly Detection}\label{sec:challenges} \todo{prepsat prvni dva odstavce lepe}
...
\begin{equation}
\textrm{cos}(a, b) = \frac{\sum\limits_{s \in S} F_s^a F_s^b} {\sqrt{\sum\limits_{s \in S} (F_s^a)^2} \sqrt{\sum\limits_{s \in S} (F_s^b)^2}}\:, \tag{2}
\end{equation}
...
Algorithm~\ref{alg:kmeans} shows the pseudocode of $K$-means.
...
The clustering as explained in this section has restrictions that make it make it inaplicable in Cognitive Targeted Anomaly Detection Framework.
...
The similarity measure that is used in this thesis as explained in Section~\ref{sec:ctadf} does not fulfill the last point of the definition above. Furthermore, it is a pairwise similarity that forms a subspace for comparing each pair of samples.
...
Let $X$ be a data set, $d : X \times X \to \mathbb{R} +$ be a distance measure (not necessarily a metric), and $\delta$ be a distance threshold below which samples are considered sufficiently similar.
...
The algorithm is shown in Algorithm~\ref{alg:delta_medoids_one_shot}, this algorithm is optimized for better memory efficiency as opposed to the version in the cited paper.
...
The subroutine \textit{ReduceClusters} shown in Algorithm~\ref{alg:reduce} helps to reduce the number of selected representatives.
If a representative does not cover at least 1\% of the cluster, the routine tries to add him to the coverage of the representative that is the most similar.
...
Circles 3D and Moons are one of the hardest datasets to represent even by methods that are designed specifically for metric space.
...
The MNIST Fashion dataset \todo{citation} consist of a training set of 60,000 samples and 10,000 test samples.
...
The capture is from 24 hours of a working day and consists of \todo{count flows} flows.
...
For loading data from files and storing them in memory the Pandas library version 0.23.4 was used.\todo{link to documentation}.

%For all experiments below the

\section{Visualizable datasets}\label{sec:exp1}

The first experiment was run on the data specifically created for this thesis.
...
Alsi the ability to select theminimal number of representatives was selected.
...
A & 400 & 21 & 23 & 23 & 23 & 23 \\
B & 400 & 17 & 20 & 20 & 20 & 20 \\
C & 400 & 19 & 20 & 20 & 20 & 20 \\ \hline
\end{tabular}
